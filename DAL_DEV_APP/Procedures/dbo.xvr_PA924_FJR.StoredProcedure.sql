...
/*******************************************************************************************************
*	DAL_DEV_APP.dbo.xvr_PA924_FJR.sql
*
*   Notes:  Account Leadership FJR Report - This does the same thing as the query in the SSRS report
	...
	execute DAL_DEV_APP.dbo.xvr_PA924_FJR
...
From DAL_DEV_APP.dbo.xvr_PA924_Main with (nolock)	
...
From DAL_DEV_APP.dbo.xvr_Est_CLE CLE with (nolock)
INNER JOIN DAL_DEV_APP.dbo.xIGFunctionCode FC with (nolock)
...
FROM DAL_DEV_APP.dbo.xvr_BU96C_Actuals with (nolock)
where [Function] in (select code_ID 
						from DAL_DEV_APP.dbo.xIGFunctionCode with (nolock)
...
SELECT d.project,
	WIPOvr60Amount = SUM(CASE WHEN d.li_type NOT IN ('D', 'A') AND DateDiff(day, d.source_trx_date, GETDATE() /*@CurrentDate*/ ) >60 THEN d.amount
							ELSE 0
						END) 							
FROM DAL_DEV_APP.dbo.PJINVDET d with (nolock)
LEFT JOIN DAL_DEV_APP.dbo.PJINVHDR h with (nolock)
	ON d.draft_num = h.draft_num
INNER JOIN DAL_DEV_APP.dbo.PJPROJ p with (nolock)
	ON d.project = p.project 
INNER JOIN DAL_DEV_APP.dbo.PJACCT a with (nolock) 
	ON d.acct = a.acct
WHERE d.hold_status <> 'PG' 
	AND d.bill_status <> 'B'
	AND a.acct_group_cd NOT IN ('CM', 'FE')
	AND p.project NOT IN (SELECT JobID 
							FROM DAL_DEV_APP.dbo.xWIPAgingException with (nolock))
...
FROM DAL_DEV_APP.dbo.PJTRAN with (nolock) 
WHERE acct = 'LABOR'
Group By Project	

insert ##ip (project)
SELECT Project 
FROM DAL_DEV_APP.dbo.PJPROJ with (nolock)
...
INNER JOIN DAL_DEV_APP.dbo.PJBILL A with (nolock)
	ON ip.Project = A.Project
INNER JOIN DAL_DEV_APP.dbo.PJBILL B with (nolock)
	ON A.project_billwith = B.project_billwith 
INNER JOIN DAL_DEV_APP.dbo.PJPROJ p with (nolock)
	ON b.project = p.project
LEFT OUTER JOIN DAL_DEV_APP.dbo.xIGProdCode pc with (nolock)
	ON p.pm_id02 = pc.code_ID
LEFT OUTER JOIN DAL_DEV_APP.dbo.CUSTOMER c with (nolock)
	ON p.pm_id01 = C.CustId
LEFT OUTER JOIN DAL_DEV_APP.dbo.PJPROJEX x with (nolock)
	ON p.project = x.project
LEFT JOIN DAL_DEV_APP.dbo.xClientContact xc with (nolock)
	ON p.user2 = xc.EA_ID
...
LEFT OUTER JOIN DAL_DEV_APP.dbo.xvr_Est_ULE_Project ULE with (nolock) --Unlocked Estimate view by project				
