...
*   Usage:	
	...
	execute DENVERAPP.dbo.WorkingCapital @Period = '201601', @AgingDate = '12/01/2015', @OverheadClientHyperionCode = '4_G9_OVRHD', @SmallClientHyperionCode = '4_G9_Small'
...
(
	select UniqueKey = X.AdjdDocType + LTRIM(RTRIM(cast(X.AdjdRefNbr as varchar(10)))) ,
		Amt = ROUND(SUM(CASE WHEN X.AdjdDocType = 'AD' THEN X.AdjAmt * - 1
							ELSE X.AdjAmt
						END),2)
	from DENVERAPP.dbo.APAdjust X
	inner join DENVERAPP.dbo.APDoc XX 
...
FROM DENVERAPP.dbo.APDoc A
-- Get detail of AP Tran lines only for lines with projects
LEFT OUTER JOIN DENVERAPP.dbo.APTran B 
...
-- permissions
---------------------------------------------
grant execute on WorkingCapital to BFGROUP
go

grant execute on WorkingCapital to MSDSL
go

grant control on WorkingCapital to MSDSL
go

grant execute on WorkingCapital to MSDynamicsSL
go
