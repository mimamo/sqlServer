...
/*******************************************************************************************************
*	MIDWESTAPP.dbo.xvr_PA924_FJR.sql
*
*   Notes:  Account Leadership FJR Report - This does the same thing as the query in the SSRS report
	...
	execute MIDWESTAPP.dbo.xvr_PA924_FJR
...
From MIDWESTAPP.dbo.xvr_PA924_Main with (nolock)	
...
FROM MIDWESTAPP.dbo.xvr_BU96C_Actuals with (nolock)
where [Function] in (select code_ID 
						from MIDWESTAPP.dbo.xIGFunctionCode with (nolock)
...
SELECT d.project,
	WIPOvr60Amount = SUM(CASE WHEN d.li_type NOT IN ('D', 'A') AND DateDiff(day, d.source_trx_date, GETDATE() /*@CurrentDate*/ ) >60 THEN d.amount
							ELSE 0
						END) 							
FROM MIDWESTAPP.dbo.PJINVDET d with (nolock)
...
WHERE d.hold_status <> 'PG' 
	...
	AND p.project NOT IN (SELECT JobID 
							FROM MIDWESTAPP.dbo.xWIPAgingException with (nolock))
...
FROM MIDWESTAPP.dbo.PJTRAN with (nolock) 
WHERE acct = 'LABOR'
Group By Project	

insert ##ip (project)
SELECT Project 
FROM MIDWESTAPP.dbo.PJPROJ with (nolock)
...
INNER JOIN MIDWESTAPP.dbo.PJBILL A with (nolock)
	ON ip.Project = A.Project
INNER JOIN MIDWESTAPP.dbo.PJBILL B with (nolock)
...
LEFT OUTER JOIN MIDWESTAPP.dbo.xvr_Est_ULE_Project ULE with (nolock) --Unlocked Estimate view by project				
