...
CREATE PROCEDURE [dbo].[ArPaymentHistoryRetrieve]     
	@Company varchar(30),
	@Client varchar(max),	--> Dynamics Client Code
...
declare @ParsedClient table (Name varchar(255))


insert @ParsedClient (Name)
SELECT Name
FROM DENVERAPP.dbo.SplitString(''' + @Client + ''')
...
SELECT Invoice = LTRIM(RTRIM(A.invoice_num)),
	...
	runDate = getdate()
FROM @parsedClient p
inner join ' + @dbName + '.dbo.pjinvhdr A
	on ltrim(rtrim(p.Name)) = ltrim(rtrim(a.Customer))
